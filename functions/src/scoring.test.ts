...
test('score formula: new volunteer with 0 completed tasks gets newcomer floor pastCompletion (0.2)', () => {
  ...
  assert.equal(
    breakdown.pastCompletion,
    0.2,
    'Newcomer pastCompletion score floor must be 0.2',
  );
});
...
test('score formula: partial vs full skill credit calculation', () => {
  ...
  const fullSkillVol: UserDoc = {
    ...baseVolunteer,
    skills: ['gardening', 'lifting', 'driving'],
  };
  ...
  assert.equal(
    breakdownPartial.skill,
    0.5,
    '1 of 2 matching skills = 0.5 score',
  );
...
test('score formula: trust score weight scaling and default value', () => {
  ...
  assert.equal(
    breakdownDefault.trust,
    0.3,
    'Missing trustScore defaults to 30 -> 0.3 norm',
  );
...
test('score formula: report penalties accumulation and cap at 0.5', () => {
  const cleanVol: UserDoc = {
    ...baseVolunteer,
    openReportsCount: 0,
    warningsCount: 0,
  };
  const penalizedVol: UserDoc = {
    ...baseVolunteer,
    openReportsCount: 2,
    warningsCount: 1,
  };
  const maxPenalizedVol: UserDoc = {
    ...baseVolunteer,
    openReportsCount: 10,
    warningsCount: 10,
  };
  ...
  assert.equal(
    bMaxPenalized.reportPenalty,
    0.5,
    'Report penalty must cap at 0.5',
  );
});
...
test('score formula: total score calculation matches weighted formula exactly', () => {
  ...
  const expectedTotal =
    0.3 * 1.0 + 0.25 * 0.5 + 0.2 * 0.8 + 0.15 * 1.0 + 0.1 * 0.7 - 0.1;
  assert.equal(
    Math.round(breakdown.total * 1000),
