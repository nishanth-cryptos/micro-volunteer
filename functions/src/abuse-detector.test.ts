...
import { VIOLATION_FREEZE_THRESHOLD, VIOLATION_WINDOW_DAYS } from './abuse-detector';
...
test('Abuse detector Strike 1: triggers warning freeze when strikeCount === 0 and count >= 3', () => {
  ...
  if (violationCount >= VIOLATION_FREEZE_THRESHOLD && !isBanned && !isSuspended) {
  ...
  assert.equal(outcome, 'freeze_warning', 'First strike should result in warning-level freeze');
});
...
test('Abuse detector Strike 2+: triggers System-actor 3-day suspension when warningStrikeCount >= 1', () => {
  ...
  assert.equal(outcome, 'suspend', 'Repeated strike should escalate to System-actor suspension');
});
...
test('Task deletion constraint (§8.6): rejecting deleteTask on in_progress tasks', () => {
  ...
  if (callerUid !== customerUid) {
    ...
    rejectionError = taskStatus === 'in_progress'
