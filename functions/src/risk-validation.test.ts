...
export function validateTaskInput(data: Record<string, any>): ValidationResult {
  ...
  if (!Array.isArray(data.requiredSkills) || data.requiredSkills.length === 0 || data.requiredSkills.length > 5) {
  ...
  if (data.riskLevel !== 'low' && data.riskLevel !== 'medium') {
    errors.push("Risk level must be either 'low' or 'medium'. Prohibited/High risk levels are not permitted.");
  }

  if (typeof data.estimatedMinutes !== 'number' || data.estimatedMinutes < 5 || data.estimatedMinutes > 480) {
    errors.push('Estimated minutes must be between 5 and 480 (8 hours).');
  }

  if (typeof data.searchRadiusM !== 'number' || data.searchRadiusM < 500 || data.searchRadiusM > 10000) {
    errors.push('Search radius must be between 500m and 10000m (10 km).');
  }

  if (!data.location || typeof data.location.lat !== 'number' || data.location.lat < -90 || data.location.lat > 90 ||
      typeof data.location.lng !== 'number' || data.location.lng < -180 || data.location.lng > 180 ||
      typeof data.location.h3Cell !== 'string' || data.location.h3Cell.length === 0) {
    errors.push('Valid location coordinates and H3 cell are required.');
  }

  if (!data.description || typeof data.description.meetingPoint !== 'string' || data.description.meetingPoint.length === 0 || data.description.meetingPoint.length > 280) {
    errors.push('Meeting point description must be between 1 and 280 characters.');
  }
...
test('Risk & Task Validation: accepts valid low-risk task data', () => {
  const validLowRiskTask = {
    ...
    description: { meetingPoint: 'Outside community garden main gate', whatToBring: 'None' },
  };

  const res = validateTaskInput(validLowRiskTask);
  assert.equal(res.valid, true, `Valid low-risk task should pass validation. Errors: ${res.errors.join(', ')}`);
...
test('Risk & Task Validation: accepts valid medium-risk task (e.g. mobility assistance)', () => {
  const validMediumRiskTask = {
    ...
    searchRadiusM: 1000,
    location: { lat: 13.0202, lng: 77.6815, h3Cell: '89618926487ffff' },
    description: { meetingPoint: 'Park entrance bench', whatToBring: 'Walking stick' },
  };
...
test('Risk & Task Validation: boundary value checks on title length, estimated minutes, and search radius', () => {
  ...
  assert.equal(validateTaskInput({ ...baseTask, title: 'A'.repeat(81) }).valid, false, 'Title > 80 chars rejected');
  assert.equal(validateTaskInput({ ...baseTask, estimatedMinutes: 4 }).valid, false, 'Minutes < 5 rejected');
  assert.equal(validateTaskInput({ ...baseTask, estimatedMinutes: 481 }).valid, false, 'Minutes > 480 rejected');
  assert.equal(validateTaskInput({ ...baseTask, searchRadiusM: 499 }).valid, false, 'Radius < 500m rejected');
  assert.equal(validateTaskInput({ ...baseTask, searchRadiusM: 10001 }).valid, false, 'Radius > 10000m rejected');
});
