// Volunteer-side list of tasks they've accepted.
// Live subscription via onSnapshot. Rule allows the volunteer to read
// any task where acceptedVolunteerId == auth.uid.
// Composite index: (acceptedVolunteerId ASC, acceptedAt DESC) declared
...
export function AcceptedTasksList({ uid }: Props) {
  ...
  if (error) {
    return (
      <p role="alert" className="mt-3 text-sm text-red-700">
        Couldn’t load your accepted tasks: {error}
      </p>
    );
  }
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-neutral-500">
                  Accepted {formatWhen(row.acceptedAt)} ·{' '}
                  <span
                    className={
